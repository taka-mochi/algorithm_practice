...
void make_mat_row_and_col_all_zero_if_zero_exist(Matrix &mat)
{
	...
	// O(raw*col)
	// Dont use zero points new memory
	//  => Save zero points information to original "mat"
	for (int row = 0; row < row_max; row++)
	{
		for (int col = 0; col < col_max; col++) 
		{
			...
			{
				// save zero point information to 0th line
				mat_raw[row][0] = 0;
				mat_raw[0][col] = 0;
			}
		}
	...
	for (int r=0; r<row_max; r++)
	{
		if (mat_raw[r][0] == 0)
		{
			for (int c=0; c<col_max; c++) mat_raw[r][c] = 0;
		}
	}
	...
	for (int c=0; c<col_max; c++)
	{
		if (mat_raw[0][c] == 0)
		{
			for (int r=0; r<row_max; r++) mat_raw[r][c] = 0;
		}
	}
}
